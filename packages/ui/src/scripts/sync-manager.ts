...
function getElements(): SyncElements | null {
  ...
  return {
    ...
    codeInput: document.getElementById('sync-code-input') as HTMLInputElement | null,
...
function showState(elements: SyncElements, state: 'none' | 'active' | 'loading'): void {
  ...
  else if (state === 'active') elements.statusActive?.classList.remove('hidden');
  else if (state === 'loading') elements.loadingState?.classList.remove('hidden');
}
...
export function initSyncManager(): void {
  ...
  createButton?.addEventListener('click', async () => {
    ...
    try {
      ...
      showState(elements, 'none');
      showError(elements, error instanceof Error ? error.message : 'Unknown error');
    }
  });
